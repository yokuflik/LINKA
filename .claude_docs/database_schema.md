...
## Message content: server-side cloud model (ADR 0039 — reverses ADR 0026/0027/0034)
- E2EE was **permanently dropped** to enable future server-side full-text / vector search. `messages.content` is now always **UTF-8 plaintext**; the server has full read access. Transport security is TLS/WSS only (ADR 0012).
- `scripts/init_db.py` (non-`--drop`) now issues `DROP COLUMN IF EXISTS` for `messages.is_encrypted` / `messages.enc_header` / `chats.last_message_enc` and `DROP TABLE IF EXISTS user_public_keys`. A deployed DB needs `python3 -m scripts.init_db` run once by hand.
- `build_last_message_preview(content, type)` — no `is_encrypted` arg, no `ENCRYPTED_MESSAGE_PREVIEW` constant.
- `UserPublicKey` model + `crud_user.*_public_key` + `user_service` public-key block + `EncryptionRequiredError` are deleted. Endpoints `PUT /users/me/public-key`, `GET /users/{id}/public-key`, `GET /chats/{id}/key-bundle` removed.

## Message search — `content_tsv` (ADR 0040, `.claude_docs/search.md`)
- `messages.content_tsv tsvector` — real column on the `Message` model (create_all makes it); kept in lockstep with `content` + `media_name` by the `trg_messages_content_tsv` **BEFORE INSERT/UPDATE row trigger** on the partitioned parent (cascades to every partition). `to_tsvector('simple', ...)` — no stemming (multilingual chat). Never written by the ORM.
- Index `ix_messages_chatid_tsv` = `gin (chat_id, content_tsv)` via `btree_gin`, partial `WHERE deleted_at IS NULL AND purged_at IS NULL AND sender_id IS NOT NULL`. One index serves in-chat (`chat_id = ...`) and global (`content_tsv @@ ...`) search. New weekly partitions inherit it via `PARTITION OF`.
- DDL lives in `modules/search/ddl.py::apply_search_ddl`, called by **both** `scripts/init_db.py` (after the ALTER loop, before `ensure_partitions`) and `tests/conftest.py` (after the DEFAULT partitions). A deployed DB: `init_db` **then** `python3 -m scripts.backfill_search_tsv` (batched `UPDATE ... WHERE content_tsv IS NULL`, interrupt-safe).
- A partition **detached** by `manage_partitions.py --cold` is not searched while detached.

## Semantic vector search — `embedding` (ADR 0042, `.claude_docs/realtime_and_redis.md` + a dedicated search doc could be added if this grows)
- `messages.embedding vector(768)` (pgvector extension, `CREATE EXTENSION IF NOT EXISTS vector`) — nullable, Gemini `gemini-embedding-001`. Written only by `modules/vector_search/crud.py::update_embeddings` (raw `UPDATE ... WHERE id = :id`, never by the ORM on the send path) — NULL until the flush-on-demand queue processes the row.
- **`vector` extension must exist before `Base.metadata.create_all()`** — `scripts/init_db.py` / `tests/conftest.py` call `modules.vector_search.ddl.ensure_vector_extension` first (unlike `content_tsv`'s built-in `TSVECTOR`, `vector` is an extension type Postgres doesn't know about yet at `CREATE TABLE` time). `apply_vector_ddl` (extension + `ADD COLUMN IF NOT EXISTS`) is the deployed-DB safety net, called from the same `init_db.py` ALTER block as everything else.
- **IVFFlat index deliberately NOT built by `init_db.py`**: `CREATE INDEX ... USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)` needs representative data to compute useful cluster centroids, so `modules/vector_search/ddl.py::ensure_ivfflat_index` is only called by `scripts/seed_vector_data.py`, after that script's 1,000-message Gemini backfill completes. HNSW was rejected (ADR 0042) — its in-RAM build graph risks OOM on the 1GB t3.micro host; IVFFlat's footprint is a small multiple of the vector data itself.
- Query: `modules/vector_search/crud.py::semantic_search_messages` — cosine distance (`<=>`, matches `vector_cosine_ops`) `ORDER BY ... LIMIT`, membership enforced by an `INNER JOIN participants` (same pattern as ADR 0040's global search), same soft-delete/purge/system-message exclusion predicate as FTS.
- Redis flush-on-demand queue (not a DB concern, see realtime_and_redis.md) is what actually populates `embedding` at runtime; `scripts/seed_vector_data.py` is the one-time bulk path for demo data.
