...
- `AuthScreen`'s welcome pane = avatar + **username** (pre-filled from `body.user.username`, live availability hint) + about. No display-name field (ADR 0018). `submitWelcome` PATCHes only the changed fields (`username` only if the user edited it) then uploads the avatar then `enterApp()`; a `{reason}`-coded 409 keeps the user on the form. `skipWelcome` just calls `enterApp()` (random handle stands).
...
- `ProfileEditModal` no longer takes `onlineVisibilityOptions`; `useProfileEdit.profileForm` is `{about_text, username}`; `saveProfile` no longer touches settings. `nameKey`/`nameLabel` are optional now — the generic name field renders only when `nameKey` is set (group modal passes `nameKey="title"`; user modal omits it).
...
- `username || phone_number`, everywhere. No contact book, no `display_name`.
- `useChats.resolvePrivateChatTitle` stores `other.user.username || other.user.phone_number` in `privateChatTitles[chatId]`; `chatDisplayName` / `chatAvatarName` read it straight. `force:true` re-pull (chat open / tab focus) picks up the peer's renamed `username` (there's a `profile_updated` push too, but the re-pull is the belt-and-braces path).
- `senderLabel` / `userLabelById` / `memberDisplayName` / `currentUserNameVariants` / draft-chat computeds all do `user.username || user.phone_number` directly.
