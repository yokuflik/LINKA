...
- `message_restored` → first `carryOverImageOrientation(oldUrl, newUrl)` (delete stashes the pre-delete URL on `m._deletedMediaUrl`; restore copies its already-probed orientation onto the fresh presigned URL so the first paint isn't the 'landscape' default), then re-runs `probeMediaOrientation` on the new URL to confirm.
...
## Image bubbles (`components/MessageList.js`)
- Fixed two-shape reserved box (portrait `w-48 aspect-[3/4]` / landscape `w-64 aspect-[4/3]`), white bg placeholder, image fades in on `@load`.
- Spinner overlay (`animate-spin` ring) sits over the white placeholder until load, tracked by `imageLoaded[media_url]` (reactive map in setup, set by `markImageLoaded`).
