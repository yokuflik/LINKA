...
| `0038-remove-legacy-python-websocket-layer.md` | Plan to delete `realtime/{ws_router,connection_manager,ws_connection_registry}.py` once the gateway is canaried. The edit/delete gap (WS-only `edit_message`/`delete_message`/`restore_message`/`purge_message`, plan risk 8) is **fixed**: `POST /internal/message/{edit,delete,restore,purge}` + gateway `ClientFrame` variants + `message_ops.rs`. `presence_service`/`realtime_service`/`internal_router`/`presence_authz` stay | Proposed (pending canary) |
