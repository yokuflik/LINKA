...
- `connection_manager` keeps three parallel per-connection tracking structures: per-chat subscriptions, per-user channel (`user_events:{user_id}`), per-presence-target (`presence_events:{user_id}`). Listeners start on 0→1 subscriber, stop at 0; `disconnect()` tears all three down.
- Per-user channel is what makes a chat/group invite created *after* a WS connected reach it live (dynamic re-subscription).
- `message_service._fan_out` is **synchronous with `send_message`** (deliberate, not queued — background queue deferred on purpose).
- All chat-scoped events (`new_message`, receipts, `typing`) reuse one per-chat Redis channel. The sender's own connection receives its own events; clients filter themselves out.
...
- `_fan_out` synchronous with `send_message` — not yet revisited.
