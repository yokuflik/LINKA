...
| `.claude_docs/ai_agent.md` | **Index + current-state reference only** (split further 2026-09-26). Schema (`Agent`/`AgentToolCallLog`/`AgentKnowledgeDocument`/`Chunk`), `restrictions`/`triggers`/`active_skill`/`paused_chat_ids` shapes, the full execution-mode (10 tools) vs config-mode (3-way `builder_state` split) registry table, and the full rate-limit table. Points to the topic files below for build history/narrative - ADR 0045/0046/0047/0049/0051/0053/0057/0059, all decisions done. |
...
| `.claude_docs/ai_agent_changelog.md` | AI agent no-ADR changelog (split out of `ai_agent.md` 2026-09-26): chronological log of bug fixes/prompt tuning/in-scope UX changes - `tools.py` package split (ADR 0056), peer-typing indicator + int/string bug, ADR 0049 build detail, ADR 0047 all-decisions summary, persona tone fixes, self-triggering-loop bug, Gemini model bump, ADR 0050 reset, ADR 0051 auto-registration, history-transcript formatting, mid-turn handoff bug, BYOK disable, all-7-prompts tone rewrite, internal-id masking. |
| `.claude_docs/ai_agent_judge_and_escalation.md` | AI agent LLM Judge + escalation detail (split out of `ai_agent.md` 2026-09-26): ADR 0053 (Judge gate, fail-open, `AgentJudgeLog`), `pause_and_escalate` behavior/notices (ADR 0054/0055 era), identity-masking detail, `resolve_user` config tool. |
| `.claude_docs/ai_agent_capacity_and_budgets.md` | AI agent capacity/budget detail (split out of `ai_agent.md` 2026-09-26): ADR 0057 (`get_capacity_status` + `peek_fixed_window`), ADR 0058 (agent shares owner's WS send-message budget + `peek_sliding_window`), ADR 0059 (token-usage windows, output cap, `GET /agents/me/usage`, `UsageProgressBar.js`). |
...
| `0051-unknown-sender-auto-registration.md` | Extends ADR 0046 decision 2: once `on_unknown_sender` fires and the turn is enqueued, the chat is auto-registered into `on_specific_chats` (empty keywords + `_auto_added_at` timestamp) so the agent keeps replying to that person; permanent until manually removed; capped at `AGENT_MAX_AUTO_CHATS` (200), oldest auto-added entry evicted first (FIFO); manually-added entries never count against the cap | Accepted |
| `0052-on-any-message-trigger.md` | New stateless `on_any_message` trigger (`{enabled}`, `DEFAULT_AGENT_TRIGGERS`): wakes the agent on every message in every private (non-group) chat, no per-chat opt-in needed; groups excluded (same scope as `on_unknown_sender`); still gated by `on_time_window`/`blocked_read_chat_ids`/`paused_chat_ids`, reuses the plain hourly `agent_activation` quota (no new bucket); matched in `trigger_engine._matches_any_message`; settings UI checkbox "Reply to every new private message" | Accepted |
| `0059-agent-token-usage-windows-and-output-cap.md` | Two rolling token-usage windows per agent (5h/500k, 7d/3M, combined input+output) via Redis fixed-window counters (`modules/agents/token_budget.py`); pre-flight char-per-token estimate gates a turn before spending a Gemini call if remaining budget is too low; `generationConfig.maxOutputTokens` capped to remaining budget per call; a `MAX_TOKENS` cutoff ends the turn immediately, its partial text shown only in the owner's own config chat, never to a real chat counterpart; fixed English owner-only notice (SET NX, once per exhaustion); `GET /agents/me/usage` + drawer `UsageProgressBar.js` (two progress bars, live "Resets in" countdown) that disables the chat composer when blocked | Accepted |
| `0053-llm-judge-message-gate.md` | LLM Judge / Semantic Router pre-filter gate before the main agent turn, execution-mode external-sender turns only (not config-mode, not schedule-fired): isolated context (latest message + domain system prompt only, never chat history/tool schemas), cheap separate Gemini model + own rate bucket, structured `JudgeVerdict(is_approved, reason)` output, fail-open on judge errors, templated redirect (no 2nd LLM call) on rejection, new `AgentJudgeLog` audit table | Accepted |
| `0054-escalation-pause-auto-expiry.md` | `pause_and_escalate` pauses now auto-expire after `AGENT_ESCALATION_PAUSE_HOURS` (default 24, env-overridable); `Agent.paused_chat_ids` becomes a list of `{chat_id, paused_at, expires_at}` objects (lazy expiry, checked on read); an owner message in their own agent chat resumes only the most-recently-escalated paused chat, not all of them | Accepted |
| `0055-explicit-resume-paused-chat-tool.md` | New config-mode tool `resume_paused_chat` (Supervisor + Builder states): owner names a person by phone/username (via the existing `resolve_user` contract) to resume that one paused chat specifically, on top of (not replacing) ADR 0054's implicit most-recent-pause resume | Accepted |
| `0056-agent-tools-domain-split.md` | Pure structural split of `modules/agents/tools.py` into a package (`common.py`/`execution.py`/`config_mode.py`/`builder_handoff.py`/`schemas.py`/`dispatch.py`); `__init__.py` thin facade, no behaviour change | Accepted |
| `0057-agent-capacity-status-tool.md` | New Builder-state-only config tool `get_capacity_status`: read-only introspection of every agent rate limit (activation quota, Gemini calls/min, daily active-seconds budget, per-sender unknown-sender quota, knowledge base, schedule entries) alongside live usage, plus a rough `capacity_estimate` of new conversations/hour; new `infra/ratelimit/service.py::peek_fixed_window` (bare Redis `GET`, no increment); `BUILDER_PROMPT` now calls it before `finish_building_agent` and folds the estimate into the final summary as an approximation | Accepted |
| `0058-agent-shares-owner-send-rate-limit.md` | The agent's in-process send path (bypasses the Rust `ws_gateway`) now checks/consumes the owner's own WS `send_message` sliding-window budget (`rlsw:send_message(_burst):{owner_user_id}`) before sending - one identity, one budget, since the agent sends as the owner; retries with backoff instead of erroring, bounded by the existing per-turn timeout; new `peek_sliding_window` + `get_capacity_status`'s `send_message_quota` block surface it | Accepted |
...
`FANOUT_REWRITE_PLAN.md` (steps 1–4 all landed), `poc/composables/REFACTOR_PLAN.md` (complete), `MESSAGE_SEARCH_PLAN.md` (ADR 0040, not started).
