...
| `.claude_docs/env_handoff.md` | **Transient handoff.** Follow-up tasks after the ADR 0029/0030 refactor: fix `.gitignore` `*.md` blanket ignore, clean the CLAUDE.md ADR index (dup 0026, phantom 0008), ephemeral test DB in `conftest.py`, and deferred bottleneck items 3 (CRUD→service) & 4 (test DI). Delete once drained. |
...
| `docs/adr/0024-optional-display-name.md` | Reintroduce an **optional, free-form, any-language** `display_name` (nullable `users.display_name VARCHAR(80)`, no uniqueness/index, **never searchable**) as a presentation layer over the unique `username`. Sanitised (`user_service.sanitize_display_name` — control/bidi/zero-width stripped, NFC, capped at `DISPLAY_NAME_MAX_LEN`=50). Fallback everywhere = `display_name \|\| username \|\| phone_number`; system-message text stays `username \|\| phone`. Anti-impersonation rests on the unique non-searchable handle + unchanged system-message text (the `@username` is not shown alongside a `display_name` — product choice; `peerHandle`/`chatSubLabel` hooks exist but return empty). `UserOut` / `UserProfileUpdateIn` (a *sent* key sets/clears it) / `profile_updated` carry it; sign-up welcome form has an optional input. No migration (`ADD COLUMN IF NOT EXISTS`). Partially reverses ADR 0018. | Accepted |
...
| `docs/adr/0025-foreground-presence-gate.md` | Foreground-only presence: new WS action `presence_active {active}` → `presence_service.set_active` (wraps `mark_online`/`mark_offline`); the `presence:{uid}` set now means "foreground connections". Client sends it on `visibilitychange`/`focus`/`blur` + every reconnect; `typing`/`recording` also gated on `windowIsActive()`. Peer online/typing labels blank while the client's own socket is down. No DB/config/event change. | Accepted |
| `docs/adr/0026-client-side-e2e-encryption.md` | Client-side E2E for **text** messages: browser-only AES-256-GCM + ECDH P-256, pairwise key wrapping (Sender-Keys noted as scale path). Server stores/routes opaque ciphertext only. New `messages.is_encrypted` / `messages.enc_header` (JSONB, no migration); `content` holds base64 ciphertext; `build_last_message_preview` → constant `🔒 Encrypted message`; push body genericised. Public keys in new `user_public_keys` table (`modules/users`): `PUT /users/me/public-key`, `GET /users/{id}/public-key`, `GET /chats/{id}/key-bundle`; server rejects a JWK with `d`. Private key = non-extractable `CryptoKey` in browser IndexedDB. TOFU, no recipient-side FS, single-device — accepted. Media E2E deferred. | Accepted |
| `docs/adr/0027-e2e-encrypted-message-edits.md` | E2E-encrypted message **edits** (extends ADR 0026 to the `edit_message` path): WS `edit_message` gains optional `enc`; `crud.edit_message_content` / `edit_delete.edit_message` write `enc_header` + `is_encrypted`; `message_edited` event carries both; a plaintext edit of an already-encrypted row is rejected (`EncryptionRequiredError`, no silent downgrade). Client re-seals via `encryptFor`; `useWsRouter` decrypts the echo. No schema change / migration. | Accepted |
| `docs/adr/0028-per-user-storage-quota.md` | Per-user hard storage quota: `STORAGE_QUOTA_BYTES` (default 1 GiB) enforced at `POST .../messages/upload-ticket` → HTTP 413 `reason: storage_quota_exceeded`. New `users.storage_bytes_used BIGINT NOT NULL DEFAULT 0` (no migration), counted **per-ref not per-object** (dedup doesn't discount). `+size` on confirmed send (`_validate_media(sender_id=…)`), `−size` on irreversible purge (ADR 0021, floored at 0); soft delete does not refund. Existing users start at 0, no backfill. PoC `friendlyError` maps 413 → polite toast. | Accepted |
| `docs/adr/0026-scheduled-messages.md` | Scheduled messages: new unpartitioned `scheduled_messages` table (not `messages`); Redis ZSET `scheduled_messages:due` (score = fire epoch) polled by an in-process `realtime/fanout/scheduled_worker.py` (started in `main.py` lifespan, Postgres = source of truth, reconciled every 60s). At fire time re-uses the existing async send path (`send_queue.enqueue_outgoing_message`) with the stored `client_message_id` as the idempotency key. REST (not WS): `POST /chats/{id}/scheduled-messages`, `GET/PATCH/DELETE /scheduled-messages[/{id}]`, `scheduled_service.py`, `scheduled_write` rate bucket. Media +1 ref at schedule, deref on cancel/fire. User-channel events `scheduled_message_sent`/`_failed`/`_changed`. PoC: `[+]` attach-menu "Schedule message" → `ScheduleMessageModal.js` + `useScheduleMessage.js`. Plan: `SCHEDULED_MESSAGES_PLAN.md`. No migration (`CREATE TABLE IF NOT EXISTS`). | Accepted |
| `docs/adr/0029-centralized-settings-accessor.md` | Additive `config.settings` accessor object over the ADR-0019 sub-modules: `from config import settings` → `settings.WS_FRAME_RATE_MAX` (flat, read-only, live resolution, no new dep). Old `from config import X` kept for `realtime/` (Rust rewrite) + `scripts/`. REST `modules/` migrated **except** the 7 modules whose constants `tests/` monkeypatch (`auth_service`/`auth_router`/`messaging_router`/`scheduled_service` + the `chat_service`/`message_service` facades) — those move with the test-DI refactor. | Accepted |
| `docs/adr/0030-feature-local-api-schemas.md` | Move the 34 Pydantic models out of the monolithic `api/schemas.py` into `modules/<feature>/schemas.py` (auth/users/media/chats/messaging). `api/schemas.py` keeps only `IdStr`. Avatar + media-ticket models live in leaf `modules/media/schemas.py` (shared by users+chats+messaging); `PublicKeyOut` in `modules/users/schemas.py`. No re-export shim (would cycle). Only the 4 routers imported `api.schemas`; `tests/` never did. | Accepted |
