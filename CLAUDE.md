...
| `docs/adr/0024-optional-display-name.md` | Reintroduce an **optional, free-form, any-language** `display_name` (nullable `users.display_name VARCHAR(80)`, no uniqueness/index, **never searchable**) as a presentation layer over the unique `username`. Sanitised (`user_service.sanitize_display_name` — control/bidi/zero-width stripped, NFC, capped at `DISPLAY_NAME_MAX_LEN`=50). Fallback everywhere = `display_name \|\| username \|\| phone_number`; system-message text stays `username \|\| phone`. Impersonation kept visible: `@username` shown as a secondary line wherever another person's `display_name` appears. `UserOut` / `UserProfileUpdateIn` (a *sent* key sets/clears it) / `profile_updated` carry it; sign-up welcome form has an optional input. No migration (`ADD COLUMN IF NOT EXISTS`). Partially reverses ADR 0018. | Accepted |
